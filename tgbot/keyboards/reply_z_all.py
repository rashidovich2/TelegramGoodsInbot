...
def menu_frep(user_id):
    ...
    keyboard.row("🎁 Купить", "🌐 Продать")
    keyboard.row("🎁 Магазины", "Афиша")
    keyboard.row("👤 Профиль",  "💰 Пополнить", "🧮 Корзина")

    if user_role is None or user_role == "":
        keyboard.row("☎ Поддержка", "Я продавец", "Админ Афиши", "Партнеры")

    if user_id in get_admins():
        keyboard.row("🎁 Управление товарами 🖍", "📊 Статистика", "Партнеры")
        keyboard.row("⚙ Настройки", "🎫 Управление событиями 🖍", "🔆 Общие функции", "🔑 Платежные системы")
        keyboard.row("Запросы продавцов", "🚛 Заказы", "📊 Отчет о продажах")

    if user_id in get_shopadmins():
        #print(f'вывод меню reply_z_all.py 28')
        keyboard.row("☎ Поддержка", "Партнеры")
...
def events_frep():
    keyboard = ReplyKeyboardMarkup(resize_keyboard=True)
    keyboard.row("📁 Создать событие ➕", "📁 Изменить событие 🖍", "📁 Удалить все события ❌")
    keyboard.row("🗃 Создать место ➕", "🗃 Изменить место 🖍", "🗃 Удалить все места ❌")
    keyboard.row("🏪 Создать артиста ➕", "🏪 Изменить артиста 🖍", "🏪 Удалить всех артистов ❌")
    keyboard.row("⬅ Главное меню")
