...
def geo_2_kb(city):
    ...
    bt1 = InlineKeyboardButton('Подтвердить', callback_data=f'geo_chosen_cities#{city}')
    bt2 = InlineKeyboardButton('Выбрать из списка', callback_data='choice_city_list')
    markup.add(bt1, bt2)
    return markup
...
def geo_4_kb(info):
    ...
    markup = InlineKeyboardMarkup(row_width=1)
    for city in cities:
        button =InlineKeyboardButton(str(city[1]), callback_data=f'geo_chosen_cities#{city[0]}')
        markup.add(button)
    return markup
